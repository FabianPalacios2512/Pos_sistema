    ...
    <div class="bg-white dark:bg-[#1a1a1d] rounded-2xl shadow-2xl max-w-md w-full overflow-hidden relative transition-all duration-300">
      ...
      <template v-if="currentState === 'confirm'">
        ...
        <div class="pt-10 pb-8 px-6 text-center">
          ...
          <p class="text-sm text-gray-400 dark:text-gray-500 mt-2 font-medium uppercase tracking-wider">
            Total a Pagar
          </p>
          
          <!-- Detalles secundarios -->
          <div class="mt-4 flex items-center justify-center gap-4 text-xs text-gray-400 dark:text-gray-500">
        ...
        <div class="border-t border-gray-100 dark:border-zinc-800">
          <p class="px-6 pt-5 pb-3 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">
            ...
            <div class="flex items-center justify-between p-4">
              <div class="flex items-center gap-4">
                ...
                <div class="w-10 h-10 rounded-xl bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 flex items-center justify-center">
                  <svg v-if="paymentMethod.code === 'cash' || paymentMethod.code === 'efectivo'" class="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
                  </svg>
                  <svg v-else-if="paymentMethod.code === 'card' || paymentMethod.code === 'tarjeta'" class="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" />
                  </svg>
                  <svg v-else-if="paymentMethod.code === 'transfer' || paymentMethod.code === 'transferencia'" class="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
                  </svg>
                  <svg v-else class="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                ...
                <div>
                  ...
                  <p v-if="paymentFee > 0" class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
          ...
          <div v-if="customer" class="mx-4 mb-4">
            <div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-zinc-800/30 rounded-xl">
              <div class="w-8 h-8 bg-gray-200 dark:bg-zinc-700 rounded-lg flex items-center justify-center">
                <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p v-if="customer.email || customer.phone" class="text-xs text-gray-400 dark:text-gray-500 truncate">
        ...
        <div class="px-4 pb-4 pt-2 space-y-3">
          <button
            ...
            :disabled="processing"
            class="w-full py-4 bg-gray-900 dark:bg-white hover:bg-black dark:hover:bg-gray-100 disabled:bg-gray-300 dark:disabled:bg-zinc-700 text-white dark:text-gray-900 text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-2"
          >
          ...
          <button
            @click="$emit('close')"
            class="w-full py-3 text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 text-sm font-medium transition-colors"
          >
            Cancelar Venta
          </button>
        </div>
      ...
      <template v-else-if="currentState === 'success'">
        ...
        <button 
          @click="handleClose"
          class="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors z-10"
        >
        ...
        <div class="pt-10 pb-6 text-center">
          ...
          <div v-else class="mb-4">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Pago Exacto</p>
          </div>
        </div>
        ...
        <div class="mx-4 mb-6 p-4 bg-gray-50 dark:bg-zinc-800/50 rounded-xl border border-dashed border-gray-200 dark:border-zinc-700">
          <div class="space-y-2">
            ...
            <div class="flex justify-between text-sm">
              <span class="text-gray-500 dark:text-gray-400">Recibido</span>
            ...
            <div class="flex justify-between text-sm pt-2 border-t border-dashed border-gray-200 dark:border-zinc-700">
              ...
              <span class="font-medium text-gray-700 dark:text-gray-300">{{ paymentMethod.name }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="px-4 pb-6 space-y-3">
          ...
          <button
            ...
            class="w-full py-4 bg-gray-900 dark:bg-white hover:bg-black dark:hover:bg-gray-100 text-white dark:text-gray-900 text-sm font-bold rounded-xl transition-all uppercase tracking-wide"
          >
            Nueva Venta
          </button>
          ...
          <div class="grid grid-cols-2 gap-2">
            <button
              ...
              class="py-3 flex items-center justify-center gap-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white text-sm font-medium border border-gray-200 dark:border-zinc-700 hover:border-gray-300 dark:hover:border-zinc-600 rounded-xl transition-all"
            >
            ...
            <button
              ...
              class="py-3 flex items-center justify-center gap-2 text-gray-600 dark:text-gray-400 hover:text-emerald-600 dark:hover:text-emerald-400 text-sm font-medium border border-gray-200 dark:border-zinc-700 hover:border-emerald-200 dark:hover:border-emerald-800 rounded-xl transition-all"
            >
            ...
            <button
              ...
              class="py-3 flex items-center justify-center gap-2 text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 text-sm font-medium border border-gray-200 dark:border-zinc-700 hover:border-blue-200 dark:hover:border-blue-800 rounded-xl transition-all"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
            ...
            <button
              ...
              class="py-3 flex items-center justify-center gap-2 text-gray-600 dark:text-gray-400 hover:text-violet-600 dark:hover:text-violet-400 text-sm font-medium border border-gray-200 dark:border-zinc-700 hover:border-violet-200 dark:hover:border-violet-800 rounded-xl transition-all"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
          ...
          <button
            ...
            class="w-full py-2.5 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 text-xs font-medium transition-colors"
          >
            Ver recibo completo
          </button>
        </div>
