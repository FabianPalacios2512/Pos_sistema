<template>
  <div class="fixed inset-0 bg-black/70 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
    <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl max-w-md w-full border border-gray-200 dark:border-zinc-700 animate-scale-in max-h-[90vh] flex flex-col">
      ...
      <div class="bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-800 px-5 py-4 rounded-t-2xl flex-shrink-0">
        <div class="flex items-center justify-between">
          ...
          <button @click="$emit('close')" class="text-gray-500 hover:text-gray-700 dark:text-zinc-400 dark:hover:text-zinc-200 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-lg p-1.5 transition-all">
        ...
        <div class="bg-amber-50 dark:bg-amber-950/20 rounded-lg p-2 border border-amber-200 dark:border-amber-900/50">
          <div class="flex items-start gap-2">
            ...
            <p class="text-xs text-amber-800 dark:text-amber-400">
              Esta operación procesará el pago y generará la factura. Verifique que la información sea correcta.
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="p-4 bg-gray-50 dark:bg-zinc-800 border-t border-gray-200 dark:border-zinc-700 rounded-b-2xl flex-shrink-0">
        <div class="flex gap-2">
          <button
            @click="$emit('close')"
            class="flex-1 px-4 py-2.5 bg-white dark:bg-zinc-900 hover:bg-slate-50 dark:hover:bg-zinc-800 text-slate-600 dark:text-zinc-200 border border-slate-200 dark:border-zinc-800 text-sm font-semibold rounded-xl transition-all hover:shadow-md"
          >
            Cancelar
          </button>
          <button
            ...
            :disabled="processing"
            class="flex-1 px-4 py-2.5 bg-slate-900 dark:bg-slate-700 hover:bg-black dark:hover:bg-slate-600 disabled:bg-gray-300 disabled:dark:bg-zinc-800 disabled:cursor-not-allowed text-white text-sm font-bold rounded-xl shadow-lg shadow-slate-400/40 dark:shadow-slate-900/50 transition-all disabled:hover:shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
          >
...
const props = defineProps({
  ...
  invoiceNumber: { type: String, default: 'FACT-000000' } // Número de factura desde el backend
})
...
const invoiceNumber = computed(() => {
  // Usar el número pasado como prop desde el POS (que viene del backend)
  return props.invoiceNumber || 'FACT-000001'
})
