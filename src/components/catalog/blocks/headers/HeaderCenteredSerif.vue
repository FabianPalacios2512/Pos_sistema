<template>
  ...
  <!-- Nombre de marca centrado en serif, hamburger izquierda, lupa+bolsa derecha. -->
  <!-- Fondo siempre blanco. Línea inferior sutil. Espaciado generoso. -->
  <header
  ...
  >
    ...
    <div class="relative flex items-center justify-between h-[68px] px-4 lg:px-8">

      <!-- Izquierda: Hamburguesa (móvil) -->
      <div class="w-10 lg:w-[120px] flex items-center">
        <button
          @click="$emit('menu')"
        ...
        <button
          ...
          class="hidden lg:flex items-center gap-2 text-xs tracking-[0.12em] uppercase transition-colors hover:opacity-60"
          :style="{ color: palette.text_dark + 'aa' }"
        >
          <svg class="w-[16px] h-[16px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.2">
      ...
      <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
        ...
        <template v-else>
          ...
          <span
            v-if="storeSubtitle"
            class="mt-[3px] text-[8px] tracking-[0.28em] uppercase font-normal"
            :style="{ color: (palette.text_dark || '#111111') + '70', fontFamily: fonts.body + ', sans-serif' }"
          >{{ storeSubtitle }}</span>
        </template>
      </div>

      <!-- Derecha: Lupa (móvil) + Bolsa -->
      <div class="w-10 lg:w-[120px] flex items-center justify-end gap-1 lg:gap-2">
      ...
      <div v-if="showSearch" class="px-4 lg:px-8 pb-4 border-t border-gray-100">
        <div class="relative flex items-center max-w-lg mx-auto mt-3">
          ...
          <input
            ...
            autofocus
            class="w-full pl-7 pr-8 py-2 text-sm bg-transparent outline-none border-b-2 transition-colors duration-200"
            :style="{
              fontFamily: fonts.body + ', sans-serif',
