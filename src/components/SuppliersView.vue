<template>
  <div class="h-full flex flex-col bg-white dark:bg-[#131314]">
    <div class="flex-1 flex flex-col p-6 space-y-5 overflow-hidden">
      ...
      <!-- TAB: PROVEEDORES -->
      <div v-if="activeTab === 'suppliers'" class="flex-1 flex flex-col min-h-0 space-y-4">
      <!-- Métricas en Ribbon -->
      <div class="flex-none flex items-center gap-6 px-4 py-3 bg-[#f8f9fa] dark:bg-[#1e1f20] rounded-xl">
        <div class="flex items-center gap-3">
              ...
              <tr v-for="supplier in paginatedSuppliers" :key="supplier.id" class="hover:bg-gray-50 dark:hover:bg-[#2a2a2d] transition-colors">
                ...
                <td class="px-4 py-3 text-center">
                  <span :class="[
                    ...
                    supplier.active ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400'
                  ]">
      ...
      <div v-if="activeTab === 'orders'" class="flex-1 flex flex-col min-h-0 space-y-4">
        <!-- Métricas de Órdenes en Ribbon -->
        <div class="flex-none flex items-center gap-6 px-4 py-3 bg-[#f8f9fa] dark:bg-[#1e1f20] rounded-xl">
          <div class="flex items-center gap-3">
            <div class="w-9 h-9 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center">
              <svg class="w-4 h-4 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="px-6 py-4 space-y-6">
          ...
          <div>
            ...
            <div v-else class="space-y-2">
              <div v-for="(item, index) in orderForm.items" :key="index" class="bg-gray-50 dark:bg-[#212124] rounded-lg p-3 flex items-center gap-3">
    ...
    <div v-if="showOrderDetailModal" class="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4" @click.self="closeOrderDetailModal">
      <div class="bg-white dark:bg-[#1e1f20] rounded-xl max-w-4xl w-full max-h-[90vh] overflow-auto">
        ...
        <div class="px-6 py-4 space-y-6">
          ...
          <!-- Productos -->
          <div>
            <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Productos</h4>
            <div class="space-y-2">
              <div v-for="item in selectedOrder?.items" :key="item.id" class="bg-gray-50 dark:bg-[#212124] rounded-lg p-3">
    ...
    <div v-if="showReceiveModal" class="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4" @click.self="closeReceiveModal">
      <div class="bg-white dark:bg-[#1e1f20] rounded-xl max-w-4xl w-full max-h-[90vh] overflow-auto">
        ...
        <div class="px-6 py-4 space-y-4">
          ...
          <div class="space-y-3">
            <div v-for="(item, index) in receiveForm.items" :key="item.item_id" class="bg-gray-50 dark:bg-[#212124] rounded-lg p-4">
  ...
  methods: {
    ...
    getOrderStatusClass(status) {
      const classes = {
        ...
        cancelled: 'px-2 py-0.5 rounded text-xs font-medium bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400'
      }
