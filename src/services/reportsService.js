...
export const reportsService = {
  ...
  async getSalesData(period = 'month') {
    try {
      ...
      const now = new Date()
      const colombiaNow = new Date(now.getTime() + (colombiaOffset * 60 * 1000))
      
      
      let startDate = new Date()
      let endDate = new Date()
      
      // Calcular fecha de inicio según el período usando zona horaria Colombia
      switch (period) {
        case 'today':
          // Hoy en Colombia: desde las 00:00:00 hasta las 23:59:59
          startDate = new Date()
      ...
      const filteredInvoices = paidInvoices.filter((invoice, index) => {
        ...
        if (period === 'today') {
          // Para hoy: SOLO facturas del día actual
          const today = new Date()
      ...
      invoices.forEach((invoice, invIndex) => {
        if (invoice.items && Array.isArray(invoice.items)) {
          ...
          invoice.items.forEach((item, itemIndex) => {
            ...
            // CORRECCIÓN: Usar subtotal + tax_amount para incluir impuestos
            const subtotal = Math.abs(parseFloat(item.subtotal || (quantity * unitPrice)))
            const taxAmount = Math.abs(parseFloat(item.tax_amount || 0))
  ...
  async getLowStockProducts() {
    try {
      // Usar el endpoint específico de productos con stock bajo
      ...
      const lowStockProducts = products
        .map(product => ({
          id: product.id,
          name: product.name,
          units: product.current_stock || 0,
          threshold: product.min_stock || 0,
  ...
  async getVentasHoyColombia(userId = null) {
    try {
      const params = userId ? { user_id: userId } : {}
      const response = await apiClient.get('/dashboard/ventas-hoy', { params })
      ...
      return { total: 0, transacciones: 0 }
    }
  }
}