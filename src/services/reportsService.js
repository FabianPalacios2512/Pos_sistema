  ...
  async getSalesData(period = 'month') {
    try {
      ...
      const filteredInvoices = paidInvoices.filter((invoice, index) => {
        ...
        if (period === 'today') {
          // Para hoy: primero buscar facturas del día actual
          const today = new Date()
          const isToday = invoiceDay === today.getDate() && 
                         invoiceMonth === today.getMonth() && 
                         invoiceYear === today.getFullYear()
          
          // Si no hay facturas de hoy, buscar las más recientes (último día con datos)
          const yesterday = new Date()
          yesterday.setDate(today.getDate() - 1)
          const isYesterday = invoiceDay === yesterday.getDate() && 
                             invoiceMonth === yesterday.getMonth() && 
                             invoiceYear === yesterday.getFullYear()
          
          isInPeriod = isToday || isYesterday
