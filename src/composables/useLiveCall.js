...
export function useLiveCall(options = {}) {
  ...
  // DESARROLLO: 5 minutos de inactividad. En producción usar 40000 (40s)
...
RESTRICCIÓN CRÍTICA - ESTE USUARIO NO ES ADMINISTRADOR:
...
NO menciones la fecha ni hora al saludar. Solo dila si el usuario te la pregunta explícitamente.
...
${esTiendaNueva ? 'Esta tienda NO tiene productos registrados aún.' : ''}
${sinCategorias ? 'Esta tienda NO tiene categorías creadas aún.' : ''}
...
- Ejemplo INCORRECTO: "50 dólares" IMPORTANTE PARA PRONUNCIACIÓN:
...
REGLA CRÍTICA - DATOS EN TIEMPO REAL (NO EN CACHÉ):
...
REGLA CRÍTICA - VERIFICAR ANTES DE CONFIRMAR:
...
- crearProductoConversacional: USA ESTO para "crear producto", "agregar producto nuevo" (Si no hay categorías, te avisará y debes crear una primero)
...
IMPORTANTE: Antes de crear productos, EL SISTEMA NECESITA AL MENOS 1 CATEGORÍA.
Si no hay categorías, crearProductoConversacional te avisará con "SIN_CATEGORIAS".

CAMPOS OBLIGATORIOS (5 campos - NO PUEDES OMITIR NINGUNO):
...
5. stock - STOCK INICIAL (MUY IMPORTANTE - si es 0, el producto NO aparecerá en el POS)
...
2. Si el sistema responde "SIN_CATEGORIAS", debes:
...
6. SIEMPRE verifica que el campo se llenó correctamente mirando la respuesta
7. Los campos se llenan VISUALMENTE en el formulario - CONFIRMA que aparecen
8. NUNCA digas "ya tenemos todos los datos" si falta el STOCK
...
IMPORTANTE: En tiendas de MODA, tu rol NO es llenar el formulario automáticamente.
...
IMPORTANTE: Este módulo puede NO estar habilitado. Si intentas acceder y no está habilitado, 
...
Verificar primero si CrediTienda está habilitado - el sistema te lo dirá.
...
5. IMPORTANTE: Después de llenar campos, USA verificarFormularioProveedor() para VER qué datos están realmente en pantalla
...
REGLA CRÍTICA - VERIFICAR ANTES DE GUARDAR:
...
REGLA IMPORTANTE - NO PREGUNTAR COSTO:
...
3. SI HAY MÚLTIPLES SEDES: pregunta "¿A cuál sede va?"
...
REGLA - VER PRODUCTOS EN ÓRDENES DE COMPRA:
...
2. Usa llenarCampoUsuario para CADA campo: 
   ...
   - llenarCampoUsuario({campo: 'role_id', valor: 'Vendedor'}) OBLIGATORIO
...
4. SIEMPRE pregunta al usuario qué ROL asignar si no lo mencionó: "¿Qué rol le asigno? Tenemos: Administrador, Vendedor, Cajero..."
5. Cuando el usuario diga que está listo, usa guardarUsuario() para guardar
El sistema NO permite crear usuarios sin ROL asignado - es campo obligatorio
NUNCA uses llenarCampoCliente para usuarios - eso es para CLIENTES, no usuarios.
...
REGLA CLAVE: Sé un supervisor inteligente. No des datos fríos, interpreta:
...
REGLA CRÍTICA FACTURAS - NO USAR DATOS EN CACHÉ:
...
REGLA CLAVE: Sé un auditor profesional pero amigable. Da datos precisos e interpreta:
            ...
            try {
              ...
              if (moduloFinal === 'accounts-receivable') {
                try {
                  ...
                  console.warn('[navegarModulo] No se pudo verificar CrediTienda:', e)
                }
              }
...
• Gastos de hoy: ${formatMoney(datosGlobales.gastos.gastosHoy)}`
                ...
                case 'alertas':
                  ...
                  if (alertas.length === 0) {
                    ...
                    respuesta = `ALERTAS DE STOCK (${alertas.length} productos):\n`
...
CAJA: ${datosGlobales.caja.estado === 'abierta' ? 'Abierta' : 'Cerrada'} - ${formatMoney(datosGlobales.caja.montoActual)}`
              ...
              if (accion === 'iniciar') {
                ...
                if (llenarResult?.success) {
                  ...
                  if (faltantes.length > 0) {
                    ...
                    if (faltantes.some(f => f.toLowerCase().includes('stock'))) {
                      mensaje += ' El stock es importante para que el producto aparezca en el POS.'
                    }
                  } else {
                ...
                if (stockPreGuardar <= 0) {
                  result = { 
                    success: false, 
                    code: 'FALTA_STOCK',
                    message: 'El producto tiene stock 0. Si lo guardas así, NO aparecerá en el Punto de Venta. ¿Con cuántas unidades iniciamos el inventario?',
          ...
          // === CREDITIENDA - GESTIÓN DE CRÉDITOS ===
          // IMPORTANTE: Verificar si CrediTienda está habilitado antes de ejecutar cualquier acción
            ...
            try {
              ...
              if (resumenResult?.success && resumenResult?.datos) {
                ...
                if (d.alertas && d.alertas.length > 0) {
                  mensaje += `\n${d.alertas.length} alerta(s) activas`
                }
            ...
            try {
              ...
              if (alertasResult?.success) {
                if (alertasResult.alertas && alertasResult.alertas.length > 0) {
                  let mensaje = `Alertas de empleados:\n`
              ...
              if (invoicesResponse.success && invoicesResponse.data) {
                ...
                if (factura) {
                  ...
                  if (factura.discount_amount > 0) {
                    mensaje += `\nDescuento aplicado: ${formatMoney(factura.discount_amount)}`
                  }
