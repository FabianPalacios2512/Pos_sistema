...
class InvoiceController extends Controller
{
    ...
    public function getNextNumber(Request $request): JsonResponse
    {
        try {
            ...
            \Log::error('❌ Error al generar número de factura', [
                'error' => $e->getMessage(),
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            foreach ($data['items'] as $item) {
                ...
                $sourceWarehouseId = null; // Inicializar antes de crear el item
                ...
                // Log de movimiento de inventario para facturas
                if ($data['type'] === 'invoice' && isset($product) && isset($previousStock) && isset($newStock)) {
                    \App\Models\InventoryMovement::create([
                        ...
                        'notes' => "Factura {$invoice->number}" . ($sourceWarehouseId ? " - Almacén ID: {$sourceWarehouseId}" : ''),
                        'user_id' => auth()->id() ?? 1,
                        'movement_date' => now()
                    ]);
                }
    ...
    public function createPosInvoice(Request $request): JsonResponse
    {
        try {
            ...
            foreach ($data['items'] as $item) {
                ...
                // 🏢 MULTI-WAREHOUSE: Descontar stock PRIMERO para capturar source_warehouse_id
                $sourceWarehouseId = null; // Inicializar antes de descontar stock
    ...
    public function sendWhatsApp(Request $request): JsonResponse
    {
        try {
            ...
            $invoiceData = [
                ...
                'customer' => $invoice->customer ? $invoice->customer->name : 'Cliente Final',
    ...
    private function generateSimplePDF($data): string
    {
        try {
            ...
            $replacements = [
                ...
                '{{ customer_name }}' => $data['customer']['name'] ?? 'Cliente Final',
    ...
    private function htmlToPdfPlain($html, $data): string
    {
        ...
        $customerName = $data['customer']['name'] ?? 'Cliente Final';
        $total = number_format($data['total'] ?? 0, 0);
        $subtotal = number_format($data['subtotal'] ?? 0, 0);
    ...
    public function sendQuotationWhatsApp(Request $request): JsonResponse
    {
        try {
            ...
            $quotationData = [
                ...
                'customer' => $quotation->customer ? $quotation->customer->name : 'Cliente Final',
