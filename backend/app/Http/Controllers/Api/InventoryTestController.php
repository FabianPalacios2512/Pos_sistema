...
class InventoryTestController extends Controller
{
    public function dashboard(Request $request): JsonResponse
    {
        try {
            ...
            if (empty($productSales)) {
                // Si no hay ventas en el período actual, obtener top productos de todos los tiempos
    ...
    public function products(Request $request): JsonResponse
    {
        try {
            ...
            $products = $query->get()->map(function($product) use ($startDate, $now, $warehouseId) {
                // 🏪 Determinar stock según bodega
                if ($warehouseId) {
                    // Obtener stock de la bodega específica
                    $warehouseStock = $product->warehouses->first();
                    $currentStock = $warehouseStock ? $warehouseStock->pivot->stock : 0;
                } else {
                    // ✅ CORREGIDO: Calcular suma real de todas las bodegas
                ...
                $totalProfit = $currentStock * $unitProfit;
                ...
                $rotationDays = $this->calculateInventoryRotation($product->id, $currentStock);
                ...
                return [
                    ...
                    'current_stock' => $currentStock, // 🏪 Stock según bodega seleccionada
                    'min_stock' => $product->min_stock,
                    'max_stock' => $product->max_stock,
                    'unit' => $product->unit ?: 'unidad', // 🏍️ Unidad de medida legacy
                    'measurement_unit' => $product->measurement_unit ?: 'unit', // 🏍️ Nueva unidad de medida
                    ...
                    'margin_percentage' => $unitProfitMargin, // 📊 Alias para compatibilidad
                    'total_profit_potential' => round($totalProfit, 2),
                    'period_sales' => $periodSales,
                    'units_sold' => $periodSales['quantity'] ?? 0, // 📊 Unidades vendidas
                    'rotation_days' => $rotationDays,
                    'rotation_class' => $this->getRotationClass($rotationDays), // 📊 Clase ABC
                    'warehouse_id' => $warehouseId // 🏪 ID de bodega si se filtró
                ];
            });
                ...
                'data' => [
                    ...
                    'summary' => [
                        ...
                        'low_stock' => $products->filter(function($p) {
                            return $p['current_stock'] > 0 && $p['current_stock'] <= $p['min_stock'];
                        })->count()
                    ]
                ]
            ]);
    ...
    {
        // 🔥 Usar tabla invoice_items en vez de JSON
        $sales = DB::table('invoice_items as ii')
            ->join('invoices as i', 'ii.invoice_id', '=', 'i.id')
            ->where('i.status', 'paid') // ✅ Status 'paid' para facturas completadas
        ...
        return [
            'quantity' => $sales->total_quantity ?? 0,
            'revenue' => round($sales->total_revenue ?? 0, 2)
        ];
    }
        ...
        try {
            ...
            $customersQuery = DB::table('customers')
                ...
                ->groupBy('customers.id', 'customers.name', 'customers.email', 'customers.phone',
                         ...
                         'customers.credit_limit', 'customers.current_debt', 'customers.loyalty_points',
                         'customers.has_discount', 'customers.discount_type', 'customers.discount_value')
                ->selectRaw('
                    customers.id,
                    customers.name,
                    customers.email,
            ...
            $customers = $customersQuery->get()->map(function ($customer) use ($startDate, $endDate) {
                ...
                return [
                    ...
                    'subtotal_debt' => (float)($customer->subtotal_debt ?? 0),
                    'available_credit' => max(0, (float)($customer->credit_limit - ($customer->subtotal_debt ?? 0))),
    ...
    /**
     * Clasificación ABC según rotación de inventario
     */
    private function getRotationClass($rotationDays): string
    {
        if ($rotationDays === 0 || $rotationDays >= 999) {
            return 'C'; // Sin rotación o muy lenta
        }

        if ($rotationDays <= 30) {
            return 'A'; // Alta rotación (se agota en menos de 30 días)
        }

        if ($rotationDays <= 90) {
            return 'B'; // Rotación media (se agota entre 30-90 días)
        }

        return 'C'; // Baja rotación (más de 90 días)
    }
